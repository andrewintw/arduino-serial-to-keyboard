...
通常安裝 Raspberry Pi 的時候，都會接上滑鼠鍵盤，甚至是螢幕。這篇文章是為了研究如何直接共享筆電的螢幕滑鼠鍵盤給 Pi 使用。

雖然 VNC、Synergy（or Barrier）也可以完美地做到這些事情，但得發生在 Linux 正常啟動後，且需要事先設定正確的網路配置。如果有一天網路出了問題，你還是得老老實實地接上一組鍵盤和螢幕來除錯。

我個人將這種共享存取方式定位在 "應急使用"。雖然研究過程中有找到一些不錯的解決方案，但如果若成本太高對應急來說也不算是好的方案。
