...
通常我們安裝 Raspberry Pi 的時候，都會接上滑鼠鍵盤，甚至是螢幕。這篇文章是為了研究如何直接共享將筆電的螢幕滑鼠鍵盤給 Pi 使用。

我個人對這種共享存取方式定位在 "應急使用"。雖然 VNC、Synergy（or Barrier）也可以完美地做到這些事情，但通常得發生在 Linux 正常啟動後，且網路需要事先被設定正確。如果系統出了問題，網路不通時，能有一組原生的鍵盤和螢幕隊除錯來說是很方便的。
